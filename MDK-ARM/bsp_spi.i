...
#line 3 "..\\Lib\\BSP\\bsp_spi.c"

BSP_SPI_Status_t BSP_SPI_Bus_Init(BSP_SPI_Bus_t *bus, SPI_HandleTypeDef *hspi)
{
    if(bus == 0 || hspi == 0) return BSP_SPI_ERROR;

    bus->hspi = hspi;
    bus->mutex = xQueueCreateMutex( ( ( uint8_t ) 1U ) );
    if ((bus->mutex != 0) == 0) {vPortRaiseBASEPRI(); for( ;; );};
    bus->active_handle = 0;
...
BSP_SPI_Status_t BSP_SPI_Handle_Init(BSP_SPI_Handle_t *handle, BSP_SPI_Bus_t *bus)
{
    if(handle == 0 || bus == 0) return BSP_SPI_ERROR;

    handle->bus = bus;
    handle->callback = 0;
    handle->task_to_notify = 0;
...
BSP_SPI_Status_t BSP_SPI_LockBus(BSP_SPI_Handle_t *handle, uint32_t timeout)
{
    if(handle == 0 || handle->bus == 0) return BSP_SPI_ERROR;
    
    if(xQueueSemaphoreTake( ( handle ->bus->mutex ), ( ( ( TickType_t ) ( ( ( TickType_t ) ( timeout ) * ( TickType_t ) ((TickType_t)1000) ) / ( TickType_t ) 1000 ) ) ) ) != ( ( BaseType_t ) 1 )){
        return BSP_SPI_BUSY;
    }
...
void BSP_SPI_UnlockBus(BSP_SPI_Handle_t *handle)
{
    if(handle == 0 || handle->bus == 0) return;
    
    handle->bus->active_handle = 0;
    handle->task_to_notify = 0;
    xQueueGenericSend( ( QueueHandle_t ) ( handle ->bus->mutex ), 0, ( ( TickType_t ) 0U ), ( ( BaseType_t ) 0 ) );
}

BSP_SPI_Status_t BSP_SPI_Transmit(BSP_SPI_Handle_t *handle, uint8_t* tx_buf, uint16_t len, uint32_t timeout)
{
    if(handle == 0 || handle->bus == 0 || tx_buf == 0 || len == 0) return BSP_SPI_ERROR;
...
BSP_SPI_Status_t BSP_SPI_Receive(BSP_SPI_Handle_t *handle, uint8_t* rx_buf, uint16_t len, uint32_t timeout)
{
    if(handle == 0 || handle->bus == 0 || rx_buf == 0 || len == 0) return BSP_SPI_ERROR;
...
BSP_SPI_Status_t BSP_SPI_TransmitReceive(BSP_SPI_Handle_t *handle, uint8_t* tx_buf, uint8_t* rx_buf, uint16_t len, uint32_t timeout)
{
    if(handle == 0 || handle->bus == 0 || tx_buf == 0 || rx_buf == 0 || len == 0) return BSP_SPI_ERROR;
...
BSP_SPI_Status_t BSP_SPI_WaitDone(BSP_SPI_Handle_t *handle, uint32_t timeout)
{
    if(handle == 0) return BSP_SPI_ERROR;
    
    if(ulTaskNotifyTake(( ( BaseType_t ) 1 ), ( ( TickType_t ) ( ( ( TickType_t ) ( timeout ) * ( TickType_t ) ((TickType_t)1000) ) / ( TickType_t ) 1000 ) )) == 0) {
...
void BSP_SPI_RegisterCallback(BSP_SPI_Handle_t *handle, BSP_SPI_Callback_t callback)
{
    if(handle != 0) handle->callback = callback;
}
...
void BSP_SPI_Abort(BSP_SPI_Handle_t *handle)
{
    if(handle != 0 && handle->bus != 0) HAL_SPI_Abort(handle->bus->hspi);
}
